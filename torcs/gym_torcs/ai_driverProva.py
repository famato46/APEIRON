...
v3: due interventi minimali per ridurre lo zigzag e recuperare velocita'
  - SMOOTHING dello sterzo (EMA leggera, taglia il rumore tick-by-tick
    senza alterare la strategia del modello).
  - ACCEL BOOST quando la pista frontale e' libera (track_9 alto E
    track_8/10 simili E auto allineata): rimuove l'incertezza in uscita curva.
  - Tutti i flag sono disattivabili (USE_STEER_SMOOTHING, USE_ACCEL_BOOST).
"""
...
# =================================================================
# NUOVO: SMOOTHING STERZO (anti-zigzag)
# Filtro esponenziale: steer_smooth = alpha*steer_new + (1-alpha)*steer_prev
# alpha alto -> reattivo ma rumoroso; alpha basso -> dolce ma in ritardo.
# Applichiamo lo smoothing SOLO quando il safety net non e' attivo: in
# emergenza vogliamo reattivita' piena.
# =================================================================
USE_STEER_SMOOTHING = True
STEER_EMA_ALPHA     = 0.70     # 70% nuovo + 30% precedente

# =================================================================
# NUOVO: ACCEL BOOST in pista libera
# Quando il muro frontale (track_9) e' lontano, le scansioni laterali
# vicine al centro (track_8 e track_10) sono simili a track_9, l'auto e'
# centrata e allineata, allora il modello sta sotto-accelerando per
# eccesso di prudenza ereditato dal bot esperto. Forziamo a 1.0.
# =================================================================
USE_ACCEL_BOOST     = True
BOOST_TRACK9_MIN    = 120.0    # m: pista libera
BOOST_ANGLE_MAX     = 0.05     # rad: auto allineata
BOOST_TRACKPOS_MAX  = 0.20     # in pista, centrata
BOOST_SIDE_RATIO    = 0.80     # track_8/9 e track_10/9 > 0.8 = sezione dritta
...
try:
    ...
    for i, f in enumerate(FEATURES):
        print(f"   [{i:2d}] {f}")
    print(f"[ai_driver] Steer smoothing: {USE_STEER_SMOOTHING}  (alpha={STEER_EMA_ALPHA})")
...
# =================================================================
# NUOVO: ACCEL BOOST detector
# =================================================================

def is_open_track(track, angle, track_pos):
    """
    True se la pista davanti e' chiaramente libera:
      - track_9 (frontale) >= BOOST_TRACK9_MIN
      - track_8 e track_10 (i due adiacenti) >= 80% di track_9
        (significa pista dritta, non stiamo per entrare in una curva)
      - auto centrata e allineata
    """
    if len(track) < 19:
        return False
    ...
    while True:
        try:
            ...
            if in_recovery:
                ...
                w_log = 99.0
                steer_mlp = float('nan')
                # In recovery NON applichiamo smoothing: serve reattivita' massima
                steer_prev = steer
                ...
                # Blend con safety geometrico
                steer_rec = recovery_steer(track_pos, angle)
                w = blend_factor(track_pos, angle)
                steer_target = (1.0 - w) * steer_mlp + w * steer_rec

                # === SMOOTHING STERZO ===
                # Applichiamo l'EMA SOLO se il safety net non sta dominando
                # (w<0.5). Quando w>=0.5 vogliamo reagire subito senza filtri.
                ...
                # === ACCEL BOOST ===
                # Solo se non stiamo frenando, la pista e' libera e dritta,
                # e l'auto e' centrata: forziamo gas pieno.
            ...
            if now - last_log_t > 1.0:
                if in_recovery:
                    ...
                    tag = "BST" if boost_active else f"w={w_log:.2f}"
