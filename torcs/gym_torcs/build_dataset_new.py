...
ripulito di Fase 2 (formato lap_*: azioni 'target_*', stato marcia 'gear').
Esegue:

  1. Feature engineering: delta_track = track_18 - track_0.
  2. Selezione delle feature di input + 3 target.
  3. Split train/val/test 80/10/10 (PRIMA del bilanciamento -> niente data leakage).
  4. Bilanciamento dello sterzo SOLO sul training set.
  5. Normalizzazione StandardScaler fittata SOLO sul training set.
  6. Salvataggio di dataset_bc.csv / dataset_bc.npz / scaler.joblib / feature_config.json

NOTA FORMATO (nuovo dataset lap_*):
  - Azioni:  target_steer, target_accel, target_brake  (con prefisso target_)
  - NON esiste 'dist_from_start' -> rimossa dalle feature.
  - Stato marcia: 'gear'.

Uso:
    python build_dataset.py dataset_clean.csv
    python build_dataset.py dataset_clean.csv -o ./out/ --plot
    python build_dataset.py dataset_clean.csv --extra-tracks   # +track_1/17
"""
...
# Feature di input. Rimosso 'dist_from_start' (assente nel nuovo formato).
INPUT_FEATURES_BASE = [
    'speedX',
    'angle',
    'trackPos',
...
# Target nel nuovo formato: prefisso target_
...
def stampa_distrib_steer(df: pd.DataFrame, label: str = ""):
    ...
    print(f"\n--- Distribuzione {STEER_COL} {label} (N={len(df)}) ---")
...
def main():
    ...
    parser.add_argument('--extra-tracks', action='store_true',
                        help='Aggiunge track_1 e track_17 alle feature di input')
    ...
    if args.extra_tracks:
        input_features = ['speedX','angle','trackPos',
                          'track_0','track_1','track_4','track_9','track_14',
                          'track_17','track_18','delta_track']
        print(">> Modalita' --extra-tracks: 11 feature di input (aggiunti track_1, track_17)")
    else:
    ...
    for t in TARGETS:
        if t not in df.columns:
            print(f"ERROR: target mancante: {t}")
            sys.exit(1)
    ...
    if args.plot:
        try:
            ...
            for a, d, title, color in zip(
                ...
                a.hist(d[STEER_COL], bins=80, color=color)
