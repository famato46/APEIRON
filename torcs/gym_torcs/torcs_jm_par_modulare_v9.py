...
ANTISPIN_K_RECOVERY    = 2.0
ANTISPIN_RECOVERY_MAX_DELTA = 0.25
ZZ_ZONE_START          = 2980.0
ZZ_ZONE_END            = 3160.0
ZZ_K_YAWDAMP           = 1.6
ZZ_EMA_ALPHA           = 0.4
ZZ_MAX_DELTA           = 0.05
...
CHICANE_FINAL_START = 2960.0
CHICANE_FINAL_END   = 3140.0

CHICANE_APPROACH_START = 2960.0
CHICANE_APPROACH_END   = 3042.0

# ----------------------------------------------------------------------
# FAST SECTOR FINALE (3300 m - fine giro).
# Dopo la chicane i sensori track_9 scendono a ~130-170m per una curva
# leggerissima, ingannando la SPEED_MAP che taglia il target a ~210 km/h.
# L'umano invece tiene il gas e tocca 265 km/h.
# Soluzione: floor minimo garantito di velocita' in funzione della dfs,
# bypassando la SPEED_MAP quando la curvatura e' quasi nulla.
# Se curv < FAST_SECTOR_CURV_THRESH il target non puo' scendere sotto
# il floor interpolato dalla FAST_SECTOR_FLOOR_MAP.
# Se curv e' piu' alta (curva reale) il floor non si applica.
# ----------------------------------------------------------------------
...
def fast_sector_floor_speed(dist_from_start, curvature):
    """
    Restituisce un FLOOR minimo di velocita' nel settore finale veloce.
    Si applica solo se la curvatura e' quasi nulla (< FAST_SECTOR_CURV_THRESH).
    Fuori range o curva reale: ritorna None (= nessun floor).
    """
    if dist_from_start < FAST_SECTOR_START or dist_from_start > FAST_SECTOR_END:
        return None
    if abs(curvature) > FAST_SECTOR_CURV_THRESH:
        return None
    pts = FAST_SECTOR_FLOOR_MAP
    if dist_from_start <= pts[0][0]:
        return pts[0][1]
    if dist_from_start >= pts[-1][0]:
        return pts[-1][1]
...
def calculate_steering(S):
    ...
    _dfs_zz = S.get('distFromStart', 0.0)
    _in_zz = (ZZ_ZONE_START <= _dfs_zz <= ZZ_ZONE_END)
    _in_chicane_approach = (CHICANE_APPROACH_START <= _dfs_zz <= CHICANE_APPROACH_END)
    yaw_damp_term = 0.0
    _state['yaw_ema'] = (1.0 - ZZ_EMA_ALPHA) * _state['yaw_ema'] + ZZ_EMA_ALPHA * yaw_rate
    if _in_zz and not (is_anomalous or antispin_active):
        yaw_damp_term = -ZZ_K_YAWDAMP * _state['yaw_ema']

    if _in_chicane_approach and not (is_anomalous or antispin_active):
        t_approach = clip((_dfs_zz - CHICANE_APPROACH_START) / (CHICANE_APPROACH_END - CHICANE_APPROACH_START), 0.0, 1.0)
        tp_cap = 0.20 * (1.0 - t_approach) + 0.10 * t_approach
        target_tp = clip(target_tp, -tp_cap, tp_cap)
        cross_err = track_pos - target_tp

    raw_steer = heading_term + cross_track_term + lookahead_term + yaw_damp_term
    ...
    if _in_zz:
        max_delta = min(max_delta, ZZ_MAX_DELTA)

    if _in_chicane_approach and not (is_anomalous or antispin_active):
        max_delta = min(max_delta, 0.035)
...
if __name__ == "__main__":
    ...
    with open(csv_filename, mode='w', newline='') as f:
        ...
        print(f"   FAST SECTOR floor [{FAST_SECTOR_START:.0f}-{FAST_SECTOR_END:.0f}] m, curv_thresh={FAST_SECTOR_CURV_THRESH}")
